...
class ahp():
    ...
    def show_confronto_criteri(self, crit, g_c):
        print
        "Has juzgando la importancia de los criterios como sigue:"
        l = len(crit)
        for i in range(l):  # vale 0 --> 3
            for j in range(l):  # vale 0 --> 3
                if j == i:
                ...
                if j > i:
                    print
                    '\t', crit[i], "\tcon respeto a \t", crit[j], "\t:", g_c[i][j]
        self.separatore()
    ...
    def show_giudizio_opz_su_crit(self, opz, crit, G):
        """mostro il giudizio """
        print
        "Has juzgando las alternativas segun los criterios como sigue:"

        for j in range(len(G[0])):  # per tutti i criteri
            for i in range(len(G)):
                print
                '\t » Has juzgado', opz[i], '\t en función del criterio', crit[j], '\tcon un', G[i][j]
                # print frase
        self.separatore()
    ...
    def risultato(self, opz=['o1', 'o2'], c=['c1', 'c2', 'c3', ], \
        ...
        if verbose:
            print
            "El resultado de la elaboracion ha sido:"
        ...
        for j in range(len(opz)):
            if verbose:
                print
                "\tLa alternativa ", opz[j], "\ttiene judicio global de", "{:6.4f}".format(float(R[[j], :]))
        ...
        if verbose:
            print
            "La alternativa mejor ha resultado", opz[ris.index(max(ris))], \
        ...
        if grafico == "yes":
            ...
            plt.xticks(range(len(opz)), opz,
                       rotation=45)  # http://matplotlib.org/examples/ticks_and_spines/ticklabels_demo_rotation.html
